...
- 巻き戻しやすいように、実質的な作業は最低 2 commit を基本として分割する運用を推奨
- agent 実行の feature 作業は、repo の都合で止まらない限り commit、push、develop への merge、branch cleanup まで進める前提
...
- rollback しやすいように、作業はタスク単位で commit を分割します。
- substantive な agent 作業は、原則として最低 2 commit に分けます。1 commit で済ませるのは本当に小さい単機能変更だけです。
- docs、asset、workflow、code は無理に 1 commit へまとめず、分けられるなら分けます。
- scaffolding、実装ロジック、tests や docs のような自然な境界で分けることを優先します。
- commit title の先頭には絵文字を付けます。
- commit body は英語で、5 行以上の Markdown 風フォーマットにします。
- 短い `-m` だけで済ませず、[`templates/commit-message-template.md`](./templates/commit-message-template.md) を元に `git commit -F` で残す想定です。

## Agent 完了条件

- Codex に feature 作業の実行を頼まれた場合、既定の finish line は「コード変更を作った」で終わりません。
- feature branch 上で検証済みの commit を積み、branch を push し、`develop` に merge し、`develop` を push し、local と remote の作業 branch を削除するところまでを完了条件にします。
- 途中で止めるのは、ユーザーが明示的に pause を頼んだ場合か、repo が PR 前提で local merge を許していない場合だけです。
